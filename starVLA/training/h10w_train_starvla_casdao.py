...
class AsyncOSSSync:
    ...
    """
    def __init__(self, local_dir: str, oss_uri: str, log_path: str, poll_sec: float = 5.0):
...
class VLATrainer(TrainerUtils):
    ...
    def _init_checkpointing(self):
        ...
        if self.accelerator.is_main_process:
            ...
            oss_bucket = getattr(self.config, "oss_bucket", None)
            if oss_bucket:
                oss_uri = f"oss://{oss_bucket}/Checkpoints/{run_id}/"
                self.osssync = AsyncOSSSync(
                    self.config.output_dir,
                    oss_uri,
                    log_path=os.path.join(self.config.output_dir, "oss_sync.log"),
                )
                self.accelerator.print("🟦 oss_bucket 配置完成")
            else:
                self.osssync = None
                self.accelerator.print("🟨 oss_bucket 未配置，跳过 OSS sync 初始化")
        else:
            self.osssync = None
    ...
    def _save_checkpoint(self):
        ...
        if self.accelerator.is_main_process:
            ...
            if self.accelerator.is_main_process and self.osssync:
                started = self.osssync.request_sync()
                if started:
                    self.accelerator.print("🟦 Started background OSS sync")
                else:
                    self.accelerator.print("🟨 OSS sync running; marked pending (will sync again after finish)")
    ...
    def train(self):
        ...
        try:
            ...
            # 不管正常结束还是异常，都尽量刷完同步
            if self.accelerator.is_main_process and getattr(self, "osssync", None) is not None:
                self.accelerator.print("🟦 Exiting: waiting for pending OSS sync to complete...")
                self.osssync.close(wait=True)
