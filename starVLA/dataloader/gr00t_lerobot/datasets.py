...
def calculate_dataset_statistics(parquet_paths: list[Path]) -> dict:
    ...
    for le_modality in tqdm(all_low_dim_data.columns, desc="Processing modalities"):
        ...
        if "task_info" in le_modality:
            continue
        if le_modality in ['image', 'left_wrist_image', 'right_wrist_image']:
            continue
...
class RandomGaussianNoise:
    def __init__(self, p=0.3, mean=0.0, std=0.03):
        self.p = p
        self.mean = mean
        self.std = std
...
class RandomBlockOcclusion:
    def __init__(self, p=0.3, scale=(0.02, 0.1)):
        self.p = p
        self.scale = scale
    ...
    def __call__(self, img):
        ...
        w, h = img.size
        block_area = random.uniform(*self.scale) * w * h
        block_w = int((block_area ** 0.5) * random.uniform(0.5, 1.5))
        ...
        for i in range(y1, y1 + block_h):
            for j in range(x1, x1 + block_w):
                if i < h and j < w:
                    img.putpixel((j, i), color)
        return img

class RandomPaste:
    def __init__(self, folder, p=0.3, size_range=(0.1, 0.3)):
        self.p = p
        self.paths = glob.glob(os.path.join(folder, "*.jpg")) + \
                     glob.glob(os.path.join(folder, "*.png"))
        self.size_range = size_range

    def __call__(self, img):
        if random.random() >= self.p or len(self.paths) == 0:
            return img

        patch = Image.open(random.choice(self.paths)).convert("RGB")
        ...
        target_w = int(128 * scale)
        target_h = int(128 * scale)
        patch = patch.resize((target_w, target_h))
...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        try:
            ...
            # Require stats shape to match current modality (e.g. action with done_flag needs dim 17).
            # Old stats from before adding done_flag would have action dim 16 and cause IndexError below.
            for our_modality in ["state", "action"]:
                le_state_action_meta = getattr(le_modality_meta, our_modality)
                for subkey, meta in le_state_action_meta.items():
                    assert isinstance(meta, LeRobotStateActionMetadata)
                    le_mod = meta.original_key
                    if le_mod not in le_statistics or "mean" not in le_statistics[le_mod]:
                        raise ValueError(f"Stats missing key {le_mod}")
                    need_len = meta.end
                    have_len = len(le_statistics[le_mod]["mean"])
                    if have_len < need_len:
                        raise ValueError(
                            f"Stats dim mismatch: {le_mod} has dim {have_len}, modality requires >= {need_len} "
                            "(e.g. after adding done_flag). Will recalculate stats."
                        )
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            ...
            ## Dual-arm end-effector version
            if self.delete_pause_frame:
                # (
                #     left_delta_pos, left_gripper,
                #     right_delta_pos, right_gripper
                # ) = self._get_position_and_gripper_values_h10w_dual(data)

                # for base_index in range(trajectory_length):
                #     has_left_translation_change = np.any(np.abs(left_delta_pos[base_index]) > EPSILON)
                #     has_left_gripper_change = base_index > 0 and (left_gripper[base_index] != left_gripper[base_index - 1])

                #     has_right_translation_change = np.any(np.abs(right_delta_pos[base_index]) > EPSILON)
                #     has_right_gripper_change = base_index > 0 and (right_gripper[base_index] != right_gripper[base_index - 1])

                #     if (
                #         has_left_translation_change or has_right_translation_change or
                #         has_left_gripper_change or has_right_gripper_change
                #     ):
                #         all_steps.append((trajectory_id, base_index))
                ### for absolute 
                (
                left_pos, left_gripper,
                right_pos, right_gripper
                ) = self._get_position_and_gripper_values_h10w_dual_joint(data)

                for base_index in range(trajectory_length):
                    if base_index == 0:
                        continue  # skip first frame

                    # --- detect position change (translation only) ---
                    has_left_change = np.any(
                        np.abs(np.array(left_pos[base_index]) - np.array(left_pos[base_index - 1])) > EPSILON
                    )
                    has_right_change = np.any(
                        np.abs(np.array(right_pos[base_index]) - np.array(right_pos[base_index - 1])) > EPSILON
                    )

                    # --- detect gripper change ---
                    has_left_gripper_change = left_gripper[base_index] != left_gripper[base_index - 1]
                    has_right_gripper_change = right_gripper[base_index] != right_gripper[base_index - 1]

                    # --- record step if any meaningful change occurred ---
                    if (
                        has_left_change or has_right_change or
                        has_left_gripper_change or has_right_gripper_change
                    ):
                        all_steps.append((trajectory_id, base_index))
            else:
    ...
    def _get_position_and_gripper_values_h10w_dual(self, data: pd.DataFrame) -> tuple[list, list, list, list]:
        """
        Extract absolute EE positions (x, y, z) and gripper values
        for both left and right arms from the 18-D action representation:
        [left_ee(7), right_ee(7), left_gripper(1), right_gripper(1)].
        Ignores orientation (qw, qx, qy, qz).
        """

        if "action.ee_action" not in data.columns:
            raise ValueError(f"Missing 'action' column. Available: {data.columns.tolist()}")

        actions = np.stack(data["action.ee_action"].to_numpy())  # [T, 18]
        # if actions.shape[1] != 18:
        #     raise ValueError(f"Unexpected action shape {actions.shape}, expected (T, 18)")

        # --- Split & extract only positions ---
        left_ee_pos = actions[:, 0:3]      # left: x, y, z
        right_ee_pos = actions[:, 7:10]    # right: x, y, z
        left_gripper = actions[:, 14]
        right_gripper = actions[:, 15]

        return left_ee_pos.tolist(), left_gripper.tolist(), right_ee_pos.tolist(), right_gripper.tolist()

    def _get_position_and_gripper_values_h10w_dual_joint(self, data: pd.DataFrame) -> tuple[list, list, list, list]:
        """
        Extract absolute joint positions and gripper values
        for both left and right arms from the 16-D joint action representation:
        [left_arm(7), right_arm(7), left_gripper(1), right_gripper(1)].
        Ignores orientation (qw, qx, qy, qz).
        """

        if "action" not in data.columns or "observation.gripper" not in data.columns:
            raise ValueError(f"Missing 'action.joint_action' or 'observation.gripper' column. Available: {data.columns.tolist()}")

        actions = np.stack(data["action"].to_numpy())  # [T, 16]
        # grippers = np.stack(data["observation.gripper"].to_numpy())  # [T, 2]
        # states = np.stack(data["observation.state"].to_numpy())  # [T, D]
        # if actions.shape[1] != 16:
        #     raise ValueError(f"Unexpected action shape {actions.shape}, expected (T, 16)")
        # print("=== Debug: Dual Joint Action Extraction ===")
        # print(f"actions shape: {actions.shape}, grippers shape: {grippers.shape}")
        # print("actions:", actions)
        # print("grippers:", grippers)
        # --- Split & extract only positions ---
        left_arm_pos = actions[:, 0:7]      # left
        left_gripper = actions[:, 7]
        right_arm_pos = actions[:, 8:15]    # right
        right_gripper = actions[:, 15]
        # print("left_arm_pos:", left_arm_pos)
        # print("left_gripper:", left_gripper)
        return left_arm_pos.tolist(), left_gripper.tolist(), right_arm_pos.tolist(), right_gripper.tolist()
    ...
    def _get_position_and_gripper_values(self, data: pd.DataFrame) -> tuple[list, list]:
        ...
        position_candidates = ['effector_delta_position']
        ...
        if delta_position_values is None:
            ...
            if 'action.effector_delta_position' in data.columns:
                delta_position_values = data['action.effector_delta_position'].to_numpy().tolist()
        ...
        gripper_values = None
        gripper_candidates = ['gripper_position', 'gripper_width']
        ...
        if gripper_values is None:
            # Fallback to the old hardcoded approach if metadata approach fails
            if 'action.gripper_position' in data.columns:
                gripper_values = data['action.gripper_position'].to_numpy().tolist()
    ...
    def _get_position_and_gripper_values_dual(self, data: pd.DataFrame) -> tuple[list, list, list, list]:
        """
        Extract delta positions (x,y,z) and gripper values for both left and right arms
        from combined ee_delta_action (14) and gripper_action (2).
        """

        # --- check expected base columns ---
        if "ee_delta_action" not in data.columns or "gripper_action" not in data.columns:
            raise ValueError(f"Missing 'ee_delta_action' or 'gripper_action'. Available: {data.columns.tolist()}")

        # Convert to numpy
        ee_actions = np.stack(data["ee_delta_action"].to_numpy())  # shape [T, 14]
        grip_actions = np.stack(data["gripper_action"].to_numpy()) # shape [T, 2]

        if ee_actions.shape[1] != 14 or grip_actions.shape[1] != 2:
            raise ValueError(f"Unexpected shapes: ee={ee_actions.shape}, gripper={grip_actions.shape}")

        # --- split based on mapping ---
        left_actions = ee_actions[:, 0:7]   # first 7 dims
        right_actions = ee_actions[:, 7:14] # next 7 dims

        left_gripper = grip_actions[:, 0]   # first gripper
        right_gripper = grip_actions[:, 1]  # second gripper

        # --- extract only Δx, Δy, Δz from each (first 3 dims) ---
        left_delta_pos = left_actions[:, :3].tolist()
        right_delta_pos = right_actions[:, :3].tolist()

        return left_delta_pos, left_gripper.tolist(), right_delta_pos, right_gripper.tolist()

    def _get_position_and_gripper_values_dual_qpos(self, data: pd.DataFrame) -> tuple[list, list, list, list]:
        """
        Extract delta joint positions (Δqpos) and gripper values for dual-arm data.

        Expects:
            - "arm_delta_action": shape [12] → left(0:6), right(6:12)
            - "gripper_action": shape [2]    → left(0), right(1)
        """

        # --- check columns ---
        if "arm_delta_action" not in data.columns or "gripper_action" not in data.columns:
            raise ValueError(
                f"Missing required columns. Available: {data.columns.tolist()}"
            )

        # --- convert to numpy arrays ---
        arm_actions = np.stack(data["arm_delta_action"].to_numpy())  # shape [T, 12]
        grip_actions = np.stack(data["gripper_action"].to_numpy())   # shape [T, 2]

        if arm_actions.shape[1] != 12 or grip_actions.shape[1] != 2:
            raise ValueError(
                f"Unexpected shapes: arm_delta_action={arm_actions.shape}, gripper_action={grip_actions.shape}"
            )

        # --- split based on mapping ---
        left_arm_delta_qpos = arm_actions[:, 0:6]
        right_arm_delta_qpos = arm_actions[:, 6:12]

        left_gripper = grip_actions[:, 0]
        right_gripper = grip_actions[:, 1]

        # Convert to lists for later iteration
        return (
            left_arm_delta_qpos.tolist(),
            left_gripper.tolist(),
            right_arm_delta_qpos.tolist(),
            right_gripper.tolist(),
        )

    def _get_position_and_gripper_values_libero(self, data: pd.DataFrame) -> tuple[list, list]:
        """
        Extract delta position (x,y,z) and gripper values from a dataset
        where 'action' is stored as a single vector per row, e.g.:
        [-0.14, 0.02, -0.06, -0.01, -0.02, -0.03, 1.0].
        """
        ...
        if "action" not in data.columns:
            raise ValueError(f"No 'action' column found. Available: {data.columns.tolist()}")

        # Convert the column of lists into a numpy array
        try:
            actions = np.stack(data["action"].to_numpy())
        except Exception as e:
            raise ValueError(f"Failed to parse 'action' column. Example entry: {data['action'].iloc[0]}") from e

        # Expected layout based on your sample:
        # [Δx, Δy, Δz, rot_x, rot_y, rot_z, gripper]
        if actions.shape[1] < 7:
            raise ValueError(f"Expected at least 7 elements per action, got shape {actions.shape}")

        # Extract x,y,z deltas (first 3 dims)
        delta_position_values = actions[:, :3].tolist()

        # Extract gripper (last dim)
    ...
    def get_data_by_modality(
        ...
        if modality == "video":
            ...
            # if modality == "state":
            #     print("state:", self.get_state_or_action(trajectory_id, modality, key, base_index))
    ...
    def save_dataset_statistics(self, save_path: Path | str, format: str = "json") -> None:
        ...
        if hasattr(self.metadata.statistics, 'action') and self.metadata.statistics.action:
            ...
            for key in keys_lgrg: #reordered_keys:
...
class LeRobotMixtureDataset(Dataset):
    ...
    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        ...
        # print("================================")
        # print(f"Dataset sampling weights: {self.dataset_sampling_weights}") # [1.]
        # print(f"Dataset lengths: {self.dataset_lengths}") # 66605
        # print(f"Total datasets available: {len(self.datasets)}") # 1
        # Sample dataset
        dataset_index = rng.choice(len(self.datasets), p=self.dataset_sampling_weights)
        dataset = self.datasets[dataset_index]
        # print(f"Sampled dataset index: {dataset_index} ({dataset.dataset_name})") # 0 (libero_object_no_noops_1.0.0_lerobot)
        ...
        # print("len(dataset.all_steps):", len(dataset.all_steps)) # 66605
        # print("dataset", dataset) # libero_object_no_noops_1.0.0_lerobot (66605 steps)
        # print(f"Sampled single step index: {single_step_index}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                for video_key in dataset.modality_keys["video"]:
                    ...
                    #     # ----- 加噪声 -----
                        RandomGaussianNoise(p=0.3, mean=0, std=0.03),
                    #     # ----- 随机遮挡（Cutout）-----
                    #     RandomBlockOcclusion(p=0.3),
                    ])
                    
                    # Apply augmentation (only during training)
                    # if self.training:  # You'll need to add a training flag to your dataset
                    if random.random() < 0.4:
                        image = augmentation_transforms(image)
                    # print(f"with data augmentaion")
            ...
            if hasattr(merged_metadata.statistics, 'state') and merged_metadata.statistics.state:
                ...
                for key in state_stats.keys():
                    if key in all_used_state_keys:
                        keys_lgrg.append(key)
                    # # Original gripper handling (commented out)
                    #     if "gripper" in key.lower():
                    #         gripper_keys.append(key)
                    #     else:
                    #         non_gripper_keys.append(key)
